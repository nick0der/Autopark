...
<div class="wrapper">
    <div class="form-input" style="height: 640px">
        <div class="input-header">
            <a href="/web/repair/list"><img src="/img/arrowback.png" width="35px" style="margin-top: 9px"></a>
            <div>Repair</div>
        </div>
        <form name="repair" action="" method="POST" id="form" autocomplete="off">
            <div class="form-row">
                <div>Date repaired:</div><input id="datepicker" type="text" class="datepicker-class" required name="dateAddedName" pattern="(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))" title="yyyy-mm-dd"/>
                <script src="/js/datepicker.js"></script>
            </div>
            <div class="form-row">
                <div>Transport:</div><@spring.formSingleSelect "repairForm.transport", mavs1, ""/>
                <br>
            </div>
            <div class="form-row">
                <div>Working team:</div><@spring.formSingleSelect "repairForm.workingTeam", mavs2, ""/>
                <br>
            </div>
            <div class="form-row">
                <div>Cost:</div><@spring.formInput "repairForm.cost" "required min='0' max='1000000'" "number"/>
                <br>
            </div>
            <div class="form-row">
                <div>Engines:</div><@spring.formInput "repairForm.engines" "required min='0' max='100'" "number"/>
                <br>
            </div>
            <div class="form-row">
                <div>Transmissions:</div><@spring.formInput "repairForm.transmissions" "required min='0' max='100'" "number"/>
                <br>
            </div>
            <div class="form-row">
                <div>Bridges:</div><@spring.formInput "repairForm.bridges" "required min='0' max='100'" "number"/>
                <br>
            </div>
            <div class="form-row">
                <div>Chassis:</div><@spring.formInput "repairForm.chassis" "required min='0' max='100'" "number"/>
                <br>
            </div>
            <@spring.formInput "repairForm.dateRepaired" "required style='display:none'" "text"/>
            <br>
